...
<!-- Scrollable datatable -->
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Receptionist Data</h5>
                  ...
                  <th>Image</th>
							...
							<tbody>
									<?php foreach($recep as $t): ?>
                                            <tr class="receptionistTable" id="tablereceptionist">
                                           ...
                                           <td><?php echo $this->doctor_model->get_gender($t->gender)[0]->gender ?></td>
                                            <td><a data-toggle="modal" data-target="#my"><img src="<?php echo base_url()?>upload/receptionist/<?php echo $t->image ?>" class="img-circle"  style="height: 50px;width: 50px;"></a></td>
                                                ...
                                                <td class="center">
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            <li class="editrecep" data-id="<?php echo $t->id?>"><a data-toggle="modal" data-target="#myModal">Edit</a></li>
                                                            <li class="dltrecep" data-id="<?php echo $t->id ?>"><a>Remove</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Edit Receptionist Data</h4>
...
$('.dltrecep').on('click',function(){
  var div = $(this);
         id = $(this).data("id");
        var url = '<?php echo site_url("receptionist/delete") ?>'+'/'+id;

     $.ajax({
         url: url,
         success: function(response){
              
                div.closest('.receptionistTable').hide('slow');
         }
            });
});
         var id;
$('.editrecep').on('click',function(){
  var div = $(this);
    id = $(this).data("id");
        var url = '<?php echo site_url("receptionist/get_receptionist") ?>'+'/'+id;
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      url: '<?php echo site_url("receptionist/update_receptionist")?>'+'/'+id,
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> receptionist Updated.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('receptionist/manage') ?>");},1000) 
     ...
     $.ajax({
         ...
         success: function(response){
              var data = $.parseJSON(response);
                ...
                $('#imgmodal').attr('src', '<?php echo base_url() ?>upload/receptionist/'+data.image);
...
$('.submit').on('click', function(){
 ...
 if (fileCount > 0) 
 {
  ...
  {
            var url = '<?php echo site_url("receptionist/update") ?>'+'/'+id;
            var post  = new Object();
                ...
                $.ajax({
                    ...
                    success : function(response){
                          ...
                          window.setTimeout(function(){window.location.replace("<?php echo site_url('receptionist/manage') ?>");},1000) 
                            ...
                            if (data['error']) {
                                 $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Not Successfull!</span> receptionist Not Updated.</div>');
                          window.setTimeout(function(){window.location.replace("<?php echo site_url('receptionist/manage') ?>");},1000) 
