...
<!-- Scrollable datatable -->
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Laboratorist Data</h5>
                  ...
                  <th>Images</th>
							...
							<tbody>
									<?php foreach($lab as $t): ?>
                                            <tr class="laboratoryTable" id="tablelaboratory">
                                            ...
                                            <?php endif ?>
                                            <td><a data-toggle="modal" data-target="#my"><img src="<?php echo base_url()?>upload/laboratorist/<?php echo $t->image ?>" class="img-circle"  style="height: 50px;width: 50px;"></a></td>
                                                ...
                                                <td class="center">
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            <li class="editlab" data-id="<?php echo $t->id?>"><a data-toggle="modal" data-target="#myModal">Edit</a></li>
                                                            <li class="dltlab" data-id="<?php echo $t->id ?>"><a>Remove</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Edit Laboratorist Data</h4>
...
<div class="row">
												...
												<div class="col-md-4">
													...
													<input type="text" id="address" class="form-control" placeholder="Put strong password">
												</div>
...
<div class="row">
												...
												<div class="col-md-4">
													...
													<input type="number" id="phone_no" class="form-control" placeholder="Put strong password">
												</div>
												<div class="col-md-4">
...
$('.dltlab').on('click',function(){
  var div = $(this);
       id = $(this).data("id");
        var url = '<?php echo site_url("laboratory/delete") ?>'+'/'+id;

     $.ajax({
         url: url,
         success: function(response){
              
                div.closest('.laboratoryTable').hide('slow');
         }
            });
});
         var id;
$('.editlab').on('click',function(){
  var div = $(this);
    id = $(this).data("id");
        var url = '<?php echo site_url("laboratory/get_laboratory") ?>'+'/'+id;
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      url: '<?php echo site_url("laboratory/update_laboratory")?>'+'/'+id,
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> laboratory Updated.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('laboratory/manage') ?>");},1000) 
     ...
     $.ajax({
         ...
         success: function(response){
              ...
              $('#name').val(data.name);
                ...
                $('#imgmodal').attr('src', '<?php echo base_url() ?>upload/laboratorist/'+data.image);
...
$('.submit').on('click', function(){
 ...
 if (fileCount > 0) 
 {
  ...
  {
            var url = '<?php echo site_url("laboratory/update") ?>'+'/'+id;
            var post  = new Object();
                ...
                $.ajax({
                    ...
                    success : function(response){
                            ...
                            if (data['success']) {
                                 $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> laboratorist updated.</div>');
                          window.setTimeout(function(){window.location.replace("<?php echo site_url('laboratory/manage') ?>");},1000) 
              
                            }
                            if (data['error']) {
                                 $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Not Successfull!</span> laboratorist Not Updated.</div>');
                          window.setTimeout(function(){window.location.replace("<?php echo site_url('laboratory/manage') ?>");},1000) 
