			...
			<!-- Advanced legend -->
								...
								<div class="panel panel-flat">
									<div class="panel-heading">
										<h5 class="panel-title">Add Nurse Form</h5>
                  ...
                  <label style="font-weight:bold;">Date Of Birth:</label>
                    <div class="input-group" style="width:0px auto;">
                      ...
                      <input type="text"  class="form-control daterange-basic" id="dob" value="1985-01-01">
                    </div>
                  </div>
...
var fileCount;
  ...
  $('.selectImage').on('click',function(e){
    ...
    var url = '<?php echo site_url("nurse/add_nurse")?>';
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> nurse Entered.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('nurse') ?>");},1000) 
...
$('.submit').on('click',function(){
    var nurseData = new Object();
    nurseData.name = $('#name').val();
    nurseData.email = $('#email').val();
    nurseData.password = $('#password').val();
    nurseData.dob = $('#dob').val();
    nurseData.phone_no = $('#phone_no').val();
    nurseData.dept_id = $('#dept_id').val();
    nurseData.address = $('#address').val();
    nurseData.description = $('#description').val();  
    nurseData.created_on = $('#created_on').val();  
    nurseData.gender = $('#gender').val();	
    var url = '<?php echo site_url("nurse/add_nurse")?>';
if(!$('#name').val()){
    	new PNotify({
    		title:'Error',
