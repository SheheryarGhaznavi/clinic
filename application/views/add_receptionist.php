							...
							<!-- Advanced legend -->
								...
								<div class="panel panel-flat">
									<div class="panel-heading">
										<h5 class="panel-title">Add Receptionist Form</h5>
          ...
          <br>
											<div class="row">
												<div class="form-group">
													...
													<input type="text" id="address" class="form-control" placeholder="e.g. Akbar">
												</div>
...
var fileCount;
  ...
  $('.selectImage').on('click',function(e){
    ...
    var url = '<?php echo site_url("receptionist/add_receptionist")?>';
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> receptionist Entered.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('receptionist') ?>");},1000) 
...
$('.submit').on('click',function(){
    var receptionistData = new Object();
    receptionistData.name = $('#name').val();
    receptionistData.email = $('#email').val();
    receptionistData.password = $('#password').val();
    receptionistData.dob = $('#dob').val();
    receptionistData.phone_no = $('#phone_no').val();
    receptionistData.address = $('#address').val();
    receptionistData.dept_id = $('#dept_id').val();
    receptionistData.gender = $('#gender').val();
    receptionistData.created_on = $('#created_on').val();
    receptionistData.description = $('#description').val();	
    var url = '<?php echo site_url("receptionist/add_receptionist")?>';
if(!$('#name').val()){
    	new PNotify({
    		title:'Error',
