							...
							<!-- Advanced legend -->
								...
								<div class="panel panel-flat">
									<div class="panel-heading">
										<h5 class="panel-title">Add Laboratorist Form</h5>
...
var fileCount;
  ...
  $('.selectImage').on('click',function(e){
    ...
    var url = '<?php echo site_url("laboratory/add_laboratory")?>';
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> laboratory Entered.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('laboratory') ?>");},1000) 
...
$('.submit').on('click',function(){
    var laboratoryData = new Object();
    laboratoryData.name = $('#name').val();
    laboratoryData.email = $('#email').val();
    laboratoryData.password = $('#password').val();
    laboratoryData.dob = $('#dob').val();
    laboratoryData.gender = $('#gender').val();
    laboratoryData.dept_id = $('#dept_id').val();
    laboratoryData.phone_no = $('#phone_no').val();
    laboratoryData.created_on = $('#created_on').val();
    laboratoryData.address = $('#address').val();
    laboratoryData.description = $('#description').val();	
    var url = '<?php echo site_url("laboratory/add_laboratory")?>';
if(!$('#name').val()){
    	new PNotify({
    		title:'Error',
    ...
    $.ajax({
        ...
        success: function(response){
             var data = $.parseJSON(response);
                         if (data['success']) {
                         $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> Laboratorist Entered.</div>');
                          window.setTimeout(function(){window.location.replace("<?php echo site_url('laboratory') ?>");},1000) 
