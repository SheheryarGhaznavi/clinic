...
<!-- Scrollable datatable -->
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Doctors Data</h5>
                  ...
                  <th>Image</th>
							...
							<tbody>
									...
									<?php foreach($doc as $t): ?>
                                            
                                            	<tr class="doctorTable" id="tabledoctor">
                                           ...
                                           <td><a data-toggle="modal" data-target="#my"><img src="<?php echo base_url()?>upload/doctor/<?php echo $t->image ?>" class="img-circle"  style="height: 50px;width: 50px;"></a></td>
                                                ...
                                                <td class="center">
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            <li class="editdoc" data-id="<?php echo $t->id?>"><a data-toggle="modal" data-target="#myModal">Edit</a></li>
                                                            <li class="dltdoc" data-id="<?php echo $t->id?>"><a>Remove</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
...
<div class="row">
												...
												<div class="col-md-4">
													...
													<input type="text" id="address" class="form-control" placeholder="Put Address">
												</div>
...
$('.dltdoc').on('click',function(){
  var div = $(this);
       id = $(this).data("id");
        var url = '<?php echo site_url("doctor/delete") ?>'+'/'+id;

     $.ajax({
         url: url,
         success: function(response){
              
                div.closest('.doctorTable').hide('slow');
         }
            });
});
...
$('.editdoc').on('click',function(){
    ...
    id = $(this).data("id");
        var url = '<?php echo site_url("doctor/get_doctor") ?>'+'/'+id;
  ...
  $('.selectImage').on('click',function(e){
    ...
    imgUpdate = new Dropzone(".selectImage",
    {
      ...
      url: '<?php echo site_url("doctor/update_doctor")?>'+'/'+id,
      ...
      success: function(file, response){
          var data = $.parseJSON(response);
                          if (data['success']) {
                          $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Successfull!</span> doctor Updated.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('doctor/manage') ?>");},1000) 
     ...
     $.ajax({
         ...
         success: function(response){
              ...
              $('#name').val(data.name);
                ...
                $('#imgmodal').attr('src', '<?php echo base_url() ?>upload/doctor/'+data.image);
...
$('.submit').on('click', function(){
 ...
 if (fileCount > 0) 
 {
  ...
  else
  {
             ...
             var url = '<?php echo site_url("doctor/update") ?>'+'/'+id;
             var post  = new Object();
                 ...
                 $.ajax({
                     ...
                     success : function(response){
                           ...
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('doctor/manage') ?>");},1000) 
                             ...
                             if (data['error']) {
                                  $('.alertdiv').html('<div class="alert alert-success alert-styled-left alert-arrow-left alert-bordered"><button type="button" class="close" data-dismiss="alert"><span>&times;</span><span class="sr-only">Close</span></button><span class="text-semibold">Not Successfull!</span> Doctor Not Updated.</div>');
                           window.setTimeout(function(){window.location.replace("<?php echo site_url('doctor/manage') ?>");},1000) 
